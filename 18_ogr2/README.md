This directory contains the evaluation/training outputs from training the *pre-trained SCAN model* with *specific `train.py` inputs* to match the published values (see `train/*.config` file)  on the full published training set:
